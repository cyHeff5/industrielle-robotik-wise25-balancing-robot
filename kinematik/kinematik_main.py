...
from kinematik.ebenen_gleichungen import func_d_fur_ebene, func_z_in_ebene
...
def func_kinematik_main(pos_arm_v, pos_arm_l, pos_arm_r, ref_point, winkel_x, winkel_y, schwinge_o_l, schwinge_u_l, solver):
    ...
    winkel_x = math.radians(winkel_x) # Winkel in rad umrechnen
    ...
    while alles_ok == 0:
        ...
        if alles_ok == 1: # z-Verschiebung verwenden
            ...
            winkel_x = 0.95 * winkel_x
            winkel_y = 0.95 * winkel_y
    ...
    b = n_vek[1]
    c = n_vek[2]
    l_n_vek = math.sqrt(a**2 + b**2 + c**2)

    # Winkel zwischen Normalenvektor und Achsen
    phi_x = math.acos(a / l_n_vek)
    phi_y = math.acos(b / l_n_vek)
     # Winkel zwischen Ebene und Achsen
    theta_x = math.degrees(math.pi/2 - phi_x)
    theta_y = math.degrees(math.pi/2 - phi_y)
    ...
    return {'winkel_x': math.degrees(winkel_x), 'winkel_y': math.degrees(winkel_y),'n_vek': n_vek, 'd' : d ,'phi_servo_v': phi_servo_v, 'phi_servo_l': phi_servo_l, 'phi_servo_r': phi_servo_r }


def func_laenge(v1, v2): # Abstand zwischen zwei Punkten
